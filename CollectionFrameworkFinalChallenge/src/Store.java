...
public class Store {
    ...
    private void abandonCarts() {
        ...
        while ((abandonedCart = oldCarts.pollFirst()) != null) {
            for (String sku : abandonedCart.getProducts().keySet()) {
                ...
                item.reserveItem(abandonedCart.getProducts().get(sku));
            }
        }
    }
    ...
    private void listProductsByCategory(boolean includdeHeader, boolean includeDetail) {

        aisleInventory.keySet().forEach(k -> {
            if (includdeHeader) System.out.println("----------\n" + k + "\n----------");
            if (!includeDetail) {
